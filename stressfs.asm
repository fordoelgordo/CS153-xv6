   ...
   b:	31 db                	xor    %ebx,%ebx
{
  ...
  a7:	e8 fe 02 00 00       	call   3aa <open>
...
//    printf(fd, "%d\n", i);
  ...
  bf:	e8 c6 02 00 00       	call   38a <write>
  for(i = 0; i < 20; i++)
  ...
  close(fd);
  d1:	e8 bc 02 00 00       	call   392 <close>
  printf(1, "read\n");
  ...
  f9:	e8 ac 02 00 00       	call   3aa <open>
 ...
 10f:	e8 6e 02 00 00       	call   382 <read>
  for (i = 0; i < 20; i++)
 114:	83 eb 01             	sub    $0x1,%ebx
 117:	75 e7                	jne    100 <main+0x100>
  close(fd);
 119:	89 3c 24             	mov    %edi,(%esp)
 11c:	e8 71 02 00 00       	call   392 <close>

  wait(&exitstatus);
 121:	8d 44 24 10          	lea    0x10(%esp),%eax
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 48a:	e8 fb fe ff ff       	call   38a <write>
  while(--i >= 0)
 ...
 4be:	8d 45 10             	lea    0x10(%ebp),%eax
{
 ...
 4f3:	88 55 e2             	mov    %dl,-0x1e(%ebp)
  write(fd, &c, 1);
 4f6:	e8 8f fe ff ff       	call   38a <write>
 ...
 566:	e8 1f fe ff ff       	call   38a <write>
        putc(fd, c);
 ...
 580:	88 55 e7             	mov    %dl,-0x19(%ebp)
  write(fd, &c, 1);
 583:	e8 02 fe ff ff       	call   38a <write>
  for(i = 0; fmt[i]; i++){
 ...
 610:	e8 75 fd ff ff       	call   38a <write>
        while(*s != 0){
 ...
 640:	e8 45 fd ff ff       	call   38a <write>
 ...
 690:	e8 f5 fc ff ff       	call   38a <write>
        ap++;
...
malloc(uint nbytes)
{
 ...
 79a:	e8 53 fc ff ff       	call   3f2 <sbrk>
  if(p == (char*)-1)
