...
main(void)
{
  ...
  19:	e8 6c 03 00 00       	call   38a <open>
  ...
  2d:	e8 90 03 00 00       	call   3c2 <dup>
  dup(0);  // stderr
  32:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
  39:	e8 84 03 00 00       	call   3c2 <dup>
  3e:	66 90                	xchg   %ax,%ax
  ...
  68:	c7 04 24 d8 0a 00 00 	movl   $0xad8,(%esp)
  ...
  c1:	e8 bc 02 00 00       	call   382 <exec>
      printf(1, "init: exec sh failed\n");
 ...
 235:	31 f6                	xor    %esi,%esi
{
 ...
 253:	e8 0a 01 00 00       	call   362 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 2a6:	e8 df 00 00 00       	call   38a <open>
  if(fd < 0)
 ...
 2bb:	e8 e2 00 00 00       	call   3a2 <fstat>
  close(fd);
 2c0:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 2c3:	89 c6                	mov    %eax,%esi
  close(fd);
 2c5:	e8 a8 00 00 00       	call   372 <close>
  return r;
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 46a:	e8 fb fe ff ff       	call   36a <write>
  while(--i >= 0)
 ...
 49e:	8d 45 10             	lea    0x10(%ebp),%eax
{
 ...
 4d3:	88 55 e2             	mov    %dl,-0x1e(%ebp)
  write(fd, &c, 1);
 4d6:	e8 8f fe ff ff       	call   36a <write>
 ...
 546:	e8 1f fe ff ff       	call   36a <write>
        putc(fd, c);
 ...
 560:	88 55 e7             	mov    %dl,-0x19(%ebp)
  write(fd, &c, 1);
 563:	e8 02 fe ff ff       	call   36a <write>
  for(i = 0; fmt[i]; i++){
 ...
 5f0:	e8 75 fd ff ff       	call   36a <write>
        while(*s != 0){
 ...
 620:	e8 45 fd ff ff       	call   36a <write>
 ...
 670:	e8 f5 fc ff ff       	call   36a <write>
        ap++;
...
malloc(uint nbytes)
{
 ...
 77a:	e8 53 fc ff ff       	call   3d2 <sbrk>
  if(p == (char*)-1)
