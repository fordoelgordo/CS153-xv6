...
main(void)
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
      ...
      28:	e8 1d 0e 00 00       	call   e4a <open>
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(fork1() == 0)
      ...
      runcmd(parsecmd(buf));
    wait(&exitstatus);
      51:	c7 04 24 d0 19 00 00 	movl   $0x19d0,(%esp)
  ...
  exit(0);
      ...
      if(chdir(buf+3) < 0)
      ca:	e8 ab 0d 00 00       	call   e7a <chdir>
      ...
      f7:	e8 36 0d 00 00       	call   e32 <close>
...
000001c0 <runcmd>:
{
  ...
  switch(cmd->type){
    ...
    if(pipe(p) < 0)
     ...
     1e6:	e8 2f 0c 00 00       	call   e1a <pipe>
    ...
    close(p[0]);
     ...
     213:	e8 1a 0c 00 00       	call   e32 <close>
    close(p[1]);
     218:	8b 45 f4             	mov    -0xc(%ebp),%eax
     21b:	89 04 24             	mov    %eax,(%esp)
     21e:	e8 0f 0c 00 00       	call   e32 <close>
    wait(&exitstatus);
     223:	c7 04 24 d0 19 00 00 	movl   $0x19d0,(%esp)
     22a:	e8 db 0b 00 00       	call   e0a <wait>
    wait(&exitstatus);
     22f:	c7 04 24 d0 19 00 00 	movl   $0x19d0,(%esp)
    ...
    exec(ecmd->argv[0], ecmd->argv);
     ...
     263:	e8 da 0b 00 00       	call   e42 <exec>
    ...
    wait(&exitstatus);
     292:	c7 04 24 d0 19 00 00 	movl   $0x19d0,(%esp)
     ...
     cf5:	31 f6                	xor    %esi,%esi
{
  ...
  for(i=0; i+1 < max; ){
    ...
    cc = read(0, &c, 1);
     ...
     d13:	e8 0a 01 00 00       	call   e22 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     d66:	e8 df 00 00 00       	call   e4a <open>
  if(fd < 0)
  ...
  r = fstat(fd, st);
     ...
     d7b:	e8 e2 00 00 00       	call   e62 <fstat>
  close(fd);
     d80:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
     d83:	89 c6                	mov    %eax,%esi
  close(fd);
     d85:	e8 a8 00 00 00       	call   e32 <close>
  return r;
...
00000e12 <waitpid>:
SYSCALL(waitpid)
     e12:	b8 16 00 00 00       	mov    $0x16,%eax
     e17:	cd 40                	int    $0x40
     e19:	c3                   	ret    

00000e1a <pipe>:
SYSCALL(pipe)
     e1a:	b8 04 00 00 00       	mov    $0x4,%eax
     e1f:	cd 40                	int    $0x40
     e21:	c3                   	ret    

00000e22 <read>:
SYSCALL(read)
     e22:	b8 05 00 00 00       	mov    $0x5,%eax
     e27:	cd 40                	int    $0x40
     e29:	c3                   	ret    

00000e2a <write>:
SYSCALL(write)
     e2a:	b8 10 00 00 00       	mov    $0x10,%eax
     e2f:	cd 40                	int    $0x40
     e31:	c3                   	ret    

00000e32 <close>:
SYSCALL(close)
     e32:	b8 15 00 00 00       	mov    $0x15,%eax
     e37:	cd 40                	int    $0x40
     e39:	c3                   	ret    

00000e3a <kill>:
SYSCALL(kill)
     e3a:	b8 06 00 00 00       	mov    $0x6,%eax
     e3f:	cd 40                	int    $0x40
     e41:	c3                   	ret    

00000e42 <exec>:
SYSCALL(exec)
     e42:	b8 07 00 00 00       	mov    $0x7,%eax
     e47:	cd 40                	int    $0x40
     e49:	c3                   	ret    

00000e4a <open>:
SYSCALL(open)
     e4a:	b8 0f 00 00 00       	mov    $0xf,%eax
     e4f:	cd 40                	int    $0x40
     e51:	c3                   	ret    

00000e52 <mknod>:
SYSCALL(mknod)
     e52:	b8 11 00 00 00       	mov    $0x11,%eax
     e57:	cd 40                	int    $0x40
     e59:	c3                   	ret    

00000e5a <unlink>:
SYSCALL(unlink)
     e5a:	b8 12 00 00 00       	mov    $0x12,%eax
     e5f:	cd 40                	int    $0x40
     e61:	c3                   	ret    

00000e62 <fstat>:
SYSCALL(fstat)
     e62:	b8 08 00 00 00       	mov    $0x8,%eax
     e67:	cd 40                	int    $0x40
     e69:	c3                   	ret    

00000e6a <link>:
SYSCALL(link)
     e6a:	b8 13 00 00 00       	mov    $0x13,%eax
     e6f:	cd 40                	int    $0x40
     e71:	c3                   	ret    

00000e72 <mkdir>:
SYSCALL(mkdir)
     e72:	b8 14 00 00 00       	mov    $0x14,%eax
     e77:	cd 40                	int    $0x40
     e79:	c3                   	ret    

00000e7a <chdir>:
SYSCALL(chdir)
     e7a:	b8 09 00 00 00       	mov    $0x9,%eax
     e7f:	cd 40                	int    $0x40
     e81:	c3                   	ret    

00000e82 <dup>:
SYSCALL(dup)
     e82:	b8 0a 00 00 00       	mov    $0xa,%eax
     e87:	cd 40                	int    $0x40
     e89:	c3                   	ret    

00000e8a <getpid>:
SYSCALL(getpid)
     e8a:	b8 0b 00 00 00       	mov    $0xb,%eax
     e8f:	cd 40                	int    $0x40
     e91:	c3                   	ret    

00000e92 <sbrk>:
SYSCALL(sbrk)
     e92:	b8 0c 00 00 00       	mov    $0xc,%eax
     e97:	cd 40                	int    $0x40
     e99:	c3                   	ret    

00000e9a <sleep>:
SYSCALL(sleep)
     e9a:	b8 0d 00 00 00       	mov    $0xd,%eax
     e9f:	cd 40                	int    $0x40
     ea1:	c3                   	ret    

00000ea2 <uptime>:
SYSCALL(uptime)
     ea2:	b8 0e 00 00 00       	mov    $0xe,%eax
     ea7:	cd 40                	int    $0x40
     ea9:	c3                   	ret    
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  write(fd, &c, 1);
     ...
     f2a:	e8 fb fe ff ff       	call   e2a <write>
  while(--i >= 0)
     ...
     f5e:	8d 45 10             	lea    0x10(%ebp),%eax
{
  ...
  write(fd, &c, 1);
     f96:	e8 8f fe ff ff       	call   e2a <write>
  ...
  write(fd, &c, 1);
    ...
    1006:	e8 1f fe ff ff       	call   e2a <write>
        putc(fd, c);
  ...
  write(fd, &c, 1);
    1023:	e8 02 fe ff ff       	call   e2a <write>
  for(i = 0; fmt[i]; i++){
  ...
  write(fd, &c, 1);
    ...
    10b0:	e8 75 fd ff ff       	call   e2a <write>
        while(*s != 0){
  ...
  write(fd, &c, 1);
    ...
    10e0:	e8 45 fd ff ff       	call   e2a <write>
  ...
  write(fd, &c, 1);
    ...
    1130:	e8 f5 fc ff ff       	call   e2a <write>
        ap++;
...
malloc(uint nbytes)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    123a:	e8 53 fc ff ff       	call   e92 <sbrk>
  if(p == (char*)-1)
